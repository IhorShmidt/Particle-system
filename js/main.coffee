...
class World
  constructor: (@canvas) ->
    ...
    @canvas.addEventListener 'mousemove', ((evt) =>
      [@mouse.x, @mouse.y] = [evt.offsetX, evt.offsetY]
    ), no
    ...
    @canvas.addEventListener 'mousewheel', ((e) =>
      ...
      if @controllable instanceof ParticleSystem
        if e.shiftKey
          ...
          @controllable.particalSize = Math.max 0, @controllable.particleSize - e.wheelDelta / 100
        else
...
class Particle extends _Object
  ...
  draw: ->
    @world.globalCompositeOperation = 'lighter'
